...
__columns_gutter.onChanging = function () { updateCount(); };
...
__rows_gutter.onChanging = function () { updateCount(); };
...
__align.onChange = function () { updateCount(); };
__bounds.onChange = function () { updateCount(); };
__group.onChange = function () { updateCount(); };
...
function startAction($fitArtboard) {
    ...
    // Remember last inputs (like original Griddder)
    checkSettingFolder();
    saveSettings();

    // Refresh live totals before running
    updateCount();
    ...
    var pieces = (result && result.items) ? result.items.length : 0;
    if (pieces) {
        alert('Total pieces generated: ' + pieces);
    }
...
win.onActivate = function () { updateCount(); };
win.center();
win.show();
